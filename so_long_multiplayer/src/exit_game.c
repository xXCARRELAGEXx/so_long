...
/*   Updated: 2024/01/17 16:26:36 by rrault           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	freeall(t_data *data)
{
	...
	free(data->image.player2[0]);
	free(data->image.player2[1]);
	free(data->image.player2[2]);
	free(data->image.player2[3]);
	free(data->image.victory);
	free(data->image.defeat);
	while (data->level[i])
	{
		free(data->level[i]);
		i++;
